<template>
    <div
        class="bg-white border border-slate-200 shadow-lg shadow-slate-950/5 rounded-2xl overflow-hidden transition-all duration-300 hover:-translate-y-1 hover:scale-102 hover:shadow-xl"
    >
        <router-link :to="`/product/${product.id}`" class="block">
            <div class="relative">
                <ProductImage
                    :image="product.image"
                    :alt="product.title"
                    class="w-full mt-2 h-48 object-contain"
                />
                <div
                    class="mt-5 text-gray-700 dark:text-gray-300 px-3 py-1 rounded-full text-xs font-medium"
                >
...
export default {
    ...
    setup() {
        ...
        const wishListStore = useWishListStore(); // Ensure wishListStore is initialized
    ...
    methods: {
        ...
        addToWishList() {
            ...
            alert("Product added to wish list!"); // Optional: To notify the user
        },
        compareProduct() {
